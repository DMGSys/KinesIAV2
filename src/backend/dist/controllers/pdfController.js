...
exports.generatePatientPDF = void 0;
...
const generatePatientPDF = async (req, res) => {
    try {
        ...
        const evoluciones = await Evolucion_js_1.Evolucion.find({ pacienteId: id })
            .sort({ sesion: -1 })
        ...
        if (paciente.alergias || paciente.medicacion || paciente.antecedentes) {
            ...
            if (paciente.alergias)
                doc.text(`Alergias: ${paciente.alergias}`);
            if (paciente.medicacion)
                doc.text(`Medicación: ${paciente.medicacion}`);
            if (paciente.antecedentes)
                doc.text(`Antecedentes: ${paciente.antecedentes}`);
            doc.moveDown();
        }
...
exports.generatePatientPDF = generatePatientPDF;
